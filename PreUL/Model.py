...
class Model(nn.Module):
    def __init__(self, args, num_users, num_items, num_bundles):
        ...
        self.bundle_embedding = nn.Sequential(
    ...
    def forward(self, x, user, bundles_ids=None, bundle=False, attention_masks=None):
        ...
        bundle_representation_size = self.max_bundle_size + 1 if (bundle and self.args.op_after_transformer == "bert") else self.max_bundle_size
        user_embedding_repeated = user.repeat(1, bundle_representation_size, 1)
        if self.args.itemThenUser:
            x = torch.cat((current_items_embeddings, user_embedding_repeated), dim=2)
        else:
            x = torch.cat((user_embedding_repeated, current_items_embeddings), dim=2)
    ...
    def forward_not_efficient(self, x, user, bundle=False, attention_masks=None):
        ''' In training: the forward method rceives a tensor of shape (batch_size, max_bundle_size)
        The user shape is (batch_size, 1), 1 use id for each user
        In eval: the forward method rceives a tensor of shape (1, max_bundle_size)
        The user shape is (#users, 1), 1 use id for each user, #users can very between 1 to 99
        ...
        user = self.user_embeddings(user).reshape(current_batch_size, 1, self.embed_shape)
