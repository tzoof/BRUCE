...
from UserBert.Model import Model
...
class Experiment():
    def __init__(self, args, output_dir):
        ...
        self.args = args
        self.num_users, self.num_bundles, self.num_items = self.get_counts(args.dataset_string)
        self.item_padding_id = self.num_items
        self.user_padding_id = self.num_users

        self.model = Model(args, self.num_users, self.num_items, self.num_bundles)
    ...
    def run_experiment(self):
        ...
        for epoch in range(self.args.num_epochs):
            ...
            for batch, (items, labels) in tqdm(enumerate(self.trainDataLoader)):
                predictions = self.model(items, self.user_padding_id).squeeze()
    ...
    # users, bundles, items
    def get_counts(self, dataset_name):
